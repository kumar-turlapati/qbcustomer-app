...
export const ProductDetailsFromSearch = ({route, navigation}) => {
  ...
  const itemNameOrId = route.params.itemName;
  const byNameOrId = route.params.byNameOrId;
  ...
  const productCode =
    itemDetails && itemDetails.itemID ? itemDetails.itemID : '';
  ...
  const brandUrl =
    itemDetails && itemDetails.brandUrl ? itemDetails.brandUrl : '';
  ...
  // console.log(wishlistItems, isItemInWishlist, 'wishlistItems......');
  ...
  useEffect(() => {
    const itemDetails = async () => {
      try {
        await axios
          .post(
            CATALOG_ITEM_DETAILS.URL,
            {itemName: itemNameOrId, byNameOrId: byNameOrId},
          ...
          .catch((error) => {
            // console.log(error, '@@@@@@@@@@@@@@@@@@@@@@@@@@');
            setItemDetailsLoading(false);
            setNoDataMessage('Oops...something went wrong :(');
          });
      } catch (error) {
        // console.log(error, '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$');
        setItemDetailsLoading(false);
        setNoDataMessage('Network error. Please try again :(');
      }
    };
    itemDetails();
  }, []);
  ...
  useEffect(() => {
    // console.log(itemDetails, 'item details in use effect......');
    if (Object.keys(itemDetails).length > 0) {
      const wishlistItems =
        itemDetails && itemDetails.wishlistItems
          ? itemDetails.wishlistItems
          : [];
      const isItemInWishlist = _find(wishlistItems, {
        itemID: String(itemDetails.itemID),
      });
      // console.log(wishlistItems, isItemInWishlist);
  ...
  const renderSliderFullView = ({item}) => {
    ...
    // console.log(imageLocation, 'image url is........');
    return (
      <View>
        <ImageZoom
