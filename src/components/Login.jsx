...
export const Login = ({ navigation }) => {
  ...
  const renderWelcomeName = () => {
    return (
      <>
        ...
        <Text style={theme.viewStyles.subTextCommonStyles}>and sign in to continue</Text>
      </>
    );
  }
    ...
    return (
      <>
        ...
        <View style={{
          ...
          />
          <TextBoxSelect style={{ width: 21, height: 21, top: 15, right: 17, position: 'absolute' }} />
        </View>
        {showOTPView &&
          <TextInput
  ...
  const renderErrorMessage = () => {
    return (
      <>
        {isMobileAPIError ?
          <Text style={styles.errorTextStyles}>We are sorry. You are not authorized to use this App. Please contact XXXXXXXX for your account activation.</Text>
          :
          <TouchableOpacity activeOpacity={1} onPress={() => {
            console.log('resend otp cliced')
          }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 30, marginTop: 20 }}>
              <Text style={[styles.errorTextStyles, {
                textAlign: 'center', color: colors.BLACK, marginTop: 0,
              }]}>Didn’t receive OTP?  </Text>
              <Text style={[styles.errorTextStyles, {
                textAlign: 'center', textDecorationLine: 'underline', fontWeight: '500', marginTop: 0,
